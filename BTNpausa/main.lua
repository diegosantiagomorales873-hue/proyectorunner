-- main.lua (Runner Solar2D) - CON PAUSA Y REANUDACIÓN
...
local function bindHold(btn, key)
    btn:addEventListener("touch", function(e)
        ...
        if isPaused then return true end  -- ⏸️ No mover si está pausado
...
local JUMP_VY = -math.floor(H * 0.58)
...
local function doJump()
    ...
    if isPaused then return end  -- ⏸️ No saltar si está pausado
...
if isSimulator then
    Runtime:addEventListener("key", function(e)
        ...
        if down and e.keyName == "p" then  -- ⏸️ Tecla P para pausar (NUEVO)
            togglePause()
        end

        if down and state == "playing" and not isPaused then  -- ⏸️ Verificar pausa
...
local function onCollision(e)
    if state ~= "playing" then return end
    if isPaused then return end  -- ⏸️ No procesar colisiones si está pausado
...
local function restart()
    ...
    -- ⏸️ Reset de pausa al reiniciar (NUEVO)
    if isPaused then
        isPaused = false
        physics.start()
        pauseOverlay.isVisible = false
