-- main.lua (Runner Solar2D) - CON PAUSA + TEMPORIZADOR 60s
...
-- ============================================
-- ⏱️ VARIABLES DEL TEMPORIZADOR
-- ============================================
local GAME_TIME = 60
local timeLeft = GAME_TIME
...
local timerText = display.newText({
    ...
    text = "⏱ 60",
    x = W - UI_MARGIN,
    y = UI_MARGIN + HUD_SIZE + 10,
    font = native.systemFontBold,
    fontSize = HUD_SIZE
})
timerText.anchorX = 1
timerText.anchorY = 0
timerText:setFillColor(0.3, 1, 0.5)

-- Barra de tiempo
local timerBarWidth = math.floor(W * 0.25)
local timerBarHeight = math.floor(HUD_SIZE * 0.4)

local timerBarBg = display.newRect(gUI, W - UI_MARGIN, timerText.y + HUD_SIZE + 10, timerBarWidth, timerBarHeight)
timerBarBg:setFillColor(0.3, 0.3, 0.3, 0.5)
timerBarBg.anchorX = 1
timerBarBg.anchorY = 0

local timerBarFill = display.newRect(gUI, W - UI_MARGIN - timerBarWidth, timerText.y + HUD_SIZE + 10, timerBarWidth, timerBarHeight)
timerBarFill:setFillColor(0.3, 1, 0.5)
timerBarFill.anchorX = 0
timerBarFill.anchorY = 0

-- Reloj animado
local clockIcon = display.newText({
    parent = gUI,
    text = "⏱",
    x = W - UI_MARGIN - timerBarWidth - 15,
    y = UI_MARGIN + HUD_SIZE + 15,
    font = native.systemFont,
    fontSize = HUD_SIZE * 1.2
})
clockIcon.anchorX = 1
clockIcon.anchorY = 0.5
...
-- ⏸️ BOTÓN DE PAUSA
-- ============================================
local pauseBtnSize = math.floor(BTN_SIZE * 0.65)

local pauseBtn = display.newRoundedRect(
    gUI, 
    W - UI_MARGIN - pauseBtnSize * 0.5,
    UI_MARGIN + pauseBtnSize * 0.5,
    pauseBtnSize, 
    pauseBtnSize, 
    math.floor(pauseBtnSize * 0.2)
)
pauseBtn:setFillColor(0, 0, 0, 0.3)
pauseBtn.strokeWidth = 2
pauseBtn:setStrokeColor(1, 1, 1, 0.4)

local pauseBtnText = display.newText({
    parent = gUI,
    text = "⏸",
    x = W - UI_MARGIN - pauseBtnSize * 0.5,
    y = UI_MARGIN + pauseBtnSize * 0.5,
    font = native.systemFont,
    fontSize = math.floor(pauseBtnSize * 0.45)
})
pauseBtnText:setFillColor(1, 1, 1, 0.9)

-- ============================================
-- ⏸️ OVERLAY DE PAUSA
-- ============================================
local pauseOverlay = display.newRect(gUI, CX, CY, W, H)
pauseOverlay:setFillColor(0, 0, 0, 0.5)
pauseOverlay.isVisible = false

local pauseLabel = display.newText({
    parent = gUI,
    text = "PAUSA\n\nToca ⏸ para continuar",
    x = CX,
    y = CY,
    width = W * 0.8,
    font = native.systemFontBold,
    fontSize = math.max(36, math.floor(minSide * 0.06)),
    align = "center"
})
pauseLabel:setFillColor(1, 1, 1)
...
-- Ajuste: salto mas controlado
local JUMP_VY = -math.floor(H * 0.38)
...
local function pauseGame()
    ...
    pauseBtnText.text = "▶"
    if music then audio.pause({ channel=1 }) end
end
...
local function resumeGame()
    ...
    pauseLabel.isVisible = false
    pauseBtnText.text = "⏸"
    if music then audio.resume({ channel=1 }) end
...
local function updateTimer(dt)
    ...
    timerBarFill.width = timerBarWidth * (timeLeft / GAME_TIME)
    
    -- Cambiar color según tiempo restante
    if timeLeft <= 10 then
        -- ROJO: Últimos 10 segundos
        timerText:setFillColor(1, 0.2, 0.2)
        timerBarFill:setFillColor(1, 0.2, 0.2)
        if not timerWarning then
            timerWarning = true
            transition.to(clockIcon, { 
                xScale=1.3, yScale=1.3, 
                time=300, iterations=-1, reflect=true 
            })
        end
    elseif timeLeft <= 20 then
        -- AMARILLO: Últimos 20 segundos
        timerText:setFillColor(1, 0.85, 0.2)
        timerBarFill:setFillColor(1, 0.85, 0.2)
        timerWarning = false
    else
    ...
    -- ¿Se acabó el tiempo?
    if timeLeft <= 0 and state == "playing" then
        lose()
    end
end
...
if isSimulator then
    Runtime:addEventListener("key", function(e)
        ...
        if down and e.keyName == "p" then
            togglePause()
        end

        if down and state == "playing" and not isPaused then
...
-- ⏱️ MODIFICADA: win() con bonus de tiempo
local function win()
    state = "win"
    local timeBonus = math.floor(timeLeft)
    if timeBonus > 0 then
        endTxt.text = "¡YOU WIN!\nTiempo: " .. timeBonus .. "s\nToca REINICIAR"
    else
        endTxt.text = "¡YOU WIN!\n¡Por los pelos!\nToca REINICIAR"
...
-- ⏱️ MODIFICADA: lose() con mensaje de tiempo
local function lose()
    state = "lose"
    if timeLeft <= 0 then
        endTxt.text = "¡SE ACABÓ EL TIEMPO!\nToca REINICIAR"
    else
...
local function restart()
    ...
    -- ⏱️ Reset de temporizador al reiniciar
    timeLeft = GAME_TIME
    timerWarning = false
    timerText.text = "⏱ " .. GAME_TIME
    timerText:setFillColor(0.3, 1, 0.5)
    timerBarFill.width = timerBarWidth
    timerBarFill:setFillColor(0.3, 1, 0.5)
    transition.cancel(clockIcon)
    clockIcon.xScale = 1
    clockIcon.yScale = 1
end
